...
| Pilot generation/reproducibility | Sau clean `npm ci`, chạy `npm run pilot:generate --prefix product` rồi `npm run pilot:verify --prefix product` | Lệnh generate tự build shared runtime packages theo thứ tự, shared config/planner gọi Gateway thật, retry có giới hạn với `429/503/504`, tạo đúng bốn SVG Mermaid/C4/Graphviz/D2 và so byte/SHA-256 với baseline commit; lưu `pilot-reproducibility.json`. |
