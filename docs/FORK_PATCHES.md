...
| KFP-006 | `server/pom.xml` | Override Netty `4.2.16.Final`, Jackson Databind `2.21.4` và HttpClient5 `5.6.1` bằng dependency management để loại các High/Critical fixable được phát hiện trong image runtime mà không thay đổi API Kroki. | Maven dependency tree, Java 25 regression test, Trivy image scan và Product Compose renderer acceptance | Bỏ từng override khi Vert.x/upstream Kroki đã kéo phiên bản bằng hoặc mới hơn; kiểm tra compatibility trước mỗi rebase. |
| KFP-007 | `server/ops/docker/Dockerfile` | Build D2 `0.7.1` từ tag/commit đã pin bằng Go `1.26.5`, cập nhật các Go module có CVE và dùng binary đã vá thay artifact release cũ; đồng thời nâng GoAT builder lên Go `1.26.5`. | Xác nhận tag/commit lúc build, Trivy binary/image scan, D2 SVG/invalid-source acceptance và determinism qua restart | Tạm thời giữ patch supply-chain; quay lại artifact upstream khi D2 phát hành bản chứa dependency fixes tương đương. |
| KFP-008 | `server/ops/docker/Dockerfile` | Purge `linux-libc-dev` và chuỗi compiler/header phụ thuộc sau khi cài xong native artifacts; các package này không cần lúc render và là nguồn advisory kernel không thể vá trong user-space runtime. | Trivy không bỏ qua advisory chưa fixed; Product Compose acceptance cho SVG/PNG, error path và secure include | Candidate upstream; giữ purge ở cuối assembly để không ảnh hưởng native artifact build và rà lại khi upstream chuyển sang runtime stage tối thiểu. |
