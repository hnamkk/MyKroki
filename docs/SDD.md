...
| Quyền tối thiểu | VS Code dùng no-auth/API key; GitHub Action dùng API key hoặc OIDC; PR check không có quyền ghi. |
...
│   ├── auth.ts           Principal chung, local và API-key authenticator
│   ├── github-oidc.ts    GitHub JWT verifier, JWKS và repository policy
...
| `GitHubOidcAuthenticator` | Verify RS256 JWT từ GitHub JWKS, `iss`, custom `aud`, `exp/nbf`, `repository_id`, `workflow_ref`, `event_name`, `ref` và event-specific policy. |
| `Principal` | Chứa `subject`, `authMethod`, `scopes`, `tenantId`, `repositoryId`, `visibility` và `requestPolicy`. |
| `PolicyAuthorizer` | Allowlist theo immutable `repository_id`; kiểm tra workflow, event, ref/base/head ref; policy revoked bị từ chối. Không chứa logic render. |
...
OIDC là adapter bổ sung trong cùng authenticator chain, không phải hệ thống auth thứ hai. Action gửi GitHub OIDC JWT trực tiếp trong Bearer header của render request; Gateway xác minh và chuyển ngay thành principal có scope `diagram:render`. JWT không được chuyển xuống Kroki, cache, metrics hoặc log. JWKS có cache, refresh khi gặp `kid` mới, cooldown và timeout cấu hình được; key đã cache tiếp tục dùng được khi GitHub tạm thời lỗi, còn token cần key chưa có trả `503 OIDC_PROVIDER_UNAVAILABLE`.
...
sequenceDiagram
    ...
    loop Mỗi source thay đổi
        Action->>GW: POST /api/v1/render + Bearer GitHub JWT
        GW->>GW: Verify signature, audience, repository/workflow/event/ref policy
...
| Mục | Nội dung |
|---|---|
| Trạng thái | Accepted; OIDC đã triển khai trong Phase 5 |
| Bối cảnh | VS Code cần no-auth/API key; GitHub public fork hưởng lợi từ OIDC. |
| Quyết định | Authenticator chain tạo một `Principal` chung; Gateway xác minh trực tiếp GitHub OIDC JWT trên render route, không cấp access token trung gian; downstream authorization/rate/cache chỉ biết principal. |
...
| Hệ quả | API key và OIDC dùng cùng Bearer contract. OIDC phải cấu hình issuer, audience, JWKS và immutable repository policy; JWT không đi xuống renderer hoặc xuất hiện trong log. |
...
| `GITHUB_OIDC_ENABLED` | `false` | Bật GitHub OIDC authenticator; cho phép deployment OIDC-only không có API key |
| `GITHUB_OIDC_ISSUER` | `https://token.actions.githubusercontent.com` | Issuer bắt buộc của GitHub JWT |
| `GITHUB_OIDC_AUDIENCE` | Không có | Custom audience bắt buộc khi bật OIDC; phải trùng input Action |
| `GITHUB_OIDC_JWKS_URL` | GitHub JWKS URL | Endpoint public key; chỉ đổi cho GitHub Enterprise/test trust domain đã kiểm soát |
| `GITHUB_OIDC_REPOSITORY_POLICIES` | Không có | JSON allowlist theo repository ID, workflow ref và policy riêng cho PR/push/workflow_dispatch |
| `GITHUB_OIDC_CLOCK_TOLERANCE_SECONDS` | `30` | Clock skew cho `exp`/`nbf`, từ 0 đến 300 giây |
| `GITHUB_OIDC_JWKS_CACHE_MAX_AGE_MS` | `600000` | Thời gian cache JWKS trước refresh |
| `GITHUB_OIDC_JWKS_COOLDOWN_MS` | `30000` | Cooldown giữa các lần refresh khi gặp `kid` mới |
| `GITHUB_OIDC_JWKS_TIMEOUT_MS` | `5000` | Timeout tải JWKS |
