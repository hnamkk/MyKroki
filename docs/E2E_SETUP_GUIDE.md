...
| `Product CI` | PR/push `main` khi product, fork integration hoặc workflow product đổi; có manual | PR chạy typecheck, unit/contract, Java regression và Extension Host; Compose full chỉ chạy push `main` hoặc manual. |
| `Upstream Kroki CI` | PR/push `main` khi Java server, renderer cục bộ, build/smoke packaging đổi; có manual/call | PR chỉ build/test Java; full image build + smoke chạy push `main`, manual hoặc nightly call. |
...
Để chạy full Product Compose trước khi merge một release candidate, mở `Product CI` trong tab Actions, chọn branch candidate và dùng `Run workflow`. Job `compose` bị skip trên event PR nhưng chạy đầy đủ trên manual dispatch. Ba bằng chứng full CI cho release phải lấy từ push `main` hoặc manual dispatch, không tính PR quick gate.
